...
public class StayRepoDbTest extends AbstractDbTest {
    ...
    private void assertStay(Stay actualStay, Stay expectedStay) {
        ...
        expectedStay.getLoans().sort(Comparator.comparing(Loan::getDateOut).reversed());

        assertLoans(actualStay.getLoans(), expectedStay.getLoans());

        assertThat(actualStay.getGuest().getId()).isEqualTo(expectedStay.getGuest().getId());
    }


    private static void assertLoans(List<Loan> actualLoans, List<Loan> expectedSortedLoans) {
        assertThat(actualLoans).hasSize(expectedSortedLoans.size());
        for (int i = 0; i < actualLoans.size(); i++) {
            Loan actualLoan = actualLoans.get(i);
            Loan expectedLoan = expectedSortedLoans.get(i);

            LoanMappingAssertions.assertLoan(actualLoan, expectedLoan);
        }
    }
